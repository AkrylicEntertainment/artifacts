...
@Service
class BuildService(val nodeService: NodeService, val nodeSocketHandler: NodeSocketHandler) {
    ...
    fun startBuildOn(node: Node, build: Build) {
        ...
        nodeSocketHandler.sendMessageToNode(node.id, 1, RequestBuildMessage(build.owner, build.repository, build.commitHash))
    }
}